...
EMBED_MODEL = os.getenv("EMBED_MODEL", "textembedding-gecko@001")  # replace with actual Vertex embed name
GEN_MODEL = os.getenv("GEN_MODEL", "text-bison@001")  # replace with actual Vertex gen model
...
def embed_text(texts):
    ...
    # Note: adapt depending on exact response shape; below assume 'embedding' present
    # For clarity, many Vertex clients return embeddings in response.predictions
...
def generate_answer(prompt, context_docs, max_output_tokens=256):
    ...
    # Response parsing may require tweak depending on Vertex SDK version
